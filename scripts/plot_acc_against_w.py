...
def hamming(s1, s2):
    ...
    for i in range(len(s1)):
        ...
        if s1[i] != s2[i]:
            d+= 1
    return d + abs(len(s1)-len(s2))
clusters.append(curr_cluster)
print("num clusters:", len(clusters))
print("max coverage:", max_coverage)
encoded_strands_file = open('../data/output/baba_smol_wAnchors/EncodedStrands.txt')
encoded_strands = encoded_strands_file.readlines()
encoded_strands = list(map(lambda x: x.strip(), encoded_strands))
x = []
y = []
...
for D in tqdm(range(8,176,2)):
    ...
    for i in range(len(reconstructed_strands)):
        t += (168-hamming(reconstructed_strands[i], encoded_strands[i]))/168
    # print(reconstructed_strands[0])
    # print(encoded_strands[0])
    # print((168-hamming(reconstructed_strands[0], encoded_strands[0]))/168)
    x.append(D)
    y.append(t/255)
...
# plt.xticks(list(range(8, 176, 2)))
plt.plot(x,y)
plt.savefig("Acc_vs_w.pdf", bbox_inches='tight')
